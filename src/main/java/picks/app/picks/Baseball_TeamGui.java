...
public class Baseball_TeamGui {

    private Baseball_team bball_teams = new Baseball_team();
    private ArrayList<Node> node_list = new ArrayList<>();


    private Node display_users() {
        bball_teams.add_users();
    ...
    private Node display_teams() {
        bball_teams.add_teams(); //DELETE THIS WHEN YOU ADD USERS TO THE BOOT UP
        BackgroundFill color = new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY);
        Background background = new Background(color);
        ...
        for(Map.Entry<String[],String> teams: bball_teams.get_teams().entrySet()){
