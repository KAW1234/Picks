...
//CREATE A METHOD THAT RETURNS A MEDIA IMAGE
//AND CREATE METHODS THAT CAN RETURN DIFFERENT MEDIA IMAGES THAT REPRESENTS
//THE STATS OF THE DATABASE
//SO ESSENTIALLY THE FOOTBALL READS THE DATA FROM DATABASE AND
//THE FOOTBALLGUI DISPLAYS THE DATA THAT THE FOOTBALL CLASS RETRIEVED
...
public class Football_teamGUI {

    private Football_teams fb_teams = new Football_teams();
    ...
    private Node display_users() {
        ...
        for (String user : fb_teams.get_users()) {
    ...
    private Node display_teams() {
        fb_teams.add_teams(); //DELETE THIS WHEN YOU ADD USERS TO THE BOOT UP
        BackgroundFill color = new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY);
        Background background = new Background(color);
        ...
        for(Map.Entry<String[],String> teams: fb_teams.get_teams().entrySet()){
